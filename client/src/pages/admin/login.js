  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      ...
      // Handle forbidden (not an admin)
        ...
        setError('You do not have admin access privileges. This email is not registered as an admin.');
        setLoading(false);
      }
      // Handle other status codes
      else {
        const message = response.data?.message || 'An error occurred during the request';
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      // Handle other status codes
        ...
        const message = response.data?.message || 'Failed to resend code. Please try again.';
        console.log('Error response:', message);
        setError(message);
        setLoading(false);
      }
            ...
            <form className="space-y-6" onSubmit={handleOtpSubmit}>
              ...
              <div>
                <button
                  ...
                  className={`w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
                >
                  {isRedirecting ? 'Redirecting...' : (loading ? 'Verifying...' : 'Login')}
                </button>
              </div>
