...
export default function AdminReviews() {
  ...
  const handleApprove = async (reviewId, approved) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(
          `Review ${approved ? "approved" : "rejected"} successfully`
        );
      } else {
        // Handle rate limiting error specifically
        if (result.error && result.error.includes("Rate limited")) {
          ...
          toast.error(
            result.error ||
              `Failed to ${approved ? "approve" : "reject"} the review`
          );
        }
      }
    } catch (error) {
            ...
            <div className="bg-white shadow overflow-hidden sm:rounded-lg">
              <div className="border-b border-gray-200 px-6 py-4 bg-gray-50">
                <h3 className="text-lg font-medium text-gray-900">
                  ...
                  <span className="ml-2 text-sm text-gray-500">
                    ({getFilteredReviews().length} total)
                  </span>
                </h3>
              </div>
              ...
              <div className="h-[600px] overflow-y-auto">
                {getFilteredReviews().map((review) => (
                  <div
                    key={review._id}
                    className="border-t border-gray-200 p-6 hover:bg-gray-50"
                  >
                      ...
                      <div className="flex-1">
                            ...
                            <img
                              src={
                                review.userAvatar.startsWith("http")
                                  ? review.userAvatar
                                  : `${process.env.NEXT_PUBLIC_API_URL}${review.userAvatar}`
                              }
                              alt={review.name}
                        ...
                        {review.eventImage && (
                          <div className="mt-4">
                            <img
                              src={
                                review.eventImage.startsWith("http")
                                  ? review.eventImage
