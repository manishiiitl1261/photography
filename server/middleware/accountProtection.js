...
const loginRateLimit = (maxAttempts = 5, windowMs = 15 * 60 * 1000) => {
  ...
  return (req, res, next) => {
    // Skip rate limiting in development for convenience
    if (process.env.NODE_ENV !== "production") {
      logger.info(`[RateLimit] Skipped in development for ${req.path}`);
      return next();
    }

    // Apply rate limit only on sensitive routes
    ...
    // Create keys for both IP and email
    const ipKey = `ip:${ip}`;
    const emailKey = `email:${email}`;
    ...
    if (ipData.count > maxAttempts || emailData.count > maxAttempts) {
      logger.warn(
        `Rate limit exceeded: ${email} from IP ${ip} on path ${req.path}`
      );

      const ipResetTime = Math.ceil(
        (ipData.startTime + windowMs - now) / 1000 / 60
      );
      const emailResetTime = Math.ceil(
        (emailData.startTime + windowMs - now) / 1000 / 60
      );
      const resetTime = Math.min(ipResetTime, emailResetTime);
